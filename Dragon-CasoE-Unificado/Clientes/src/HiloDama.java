...
/**
 * HILO DAMA - CASO E: Evacuacion cuando dragonAtacando (Escenario 10).
 * Se conecta al Porton por socket con comando EVACUAR.
 */
public class HiloDama extends Thread {
    ...
    public void run() {
        while (!ClienteMaestro.simulacionTerminada) {
            // NUEVO: Evacuar si el dragon ataca
    ...
    private void evacuar() {
        ...
        try {
            ...
            sal.writeUTF("EVACUAR"); sal.writeUTF(nombreDama);
            System.out.println("[" + nombreDama + "] " + ent.readUTF());
