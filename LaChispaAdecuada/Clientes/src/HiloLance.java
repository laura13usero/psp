...
public class HiloLance extends Thread {
    ...
    private void hablarConCompaneros() {
        ...
        for (int intentos = 0; intentos < 4 && !atendido; intentos++) {
            ...
            synchronized (ClienteMaestro.lockLance) {
                if (ClienteMaestro.hayPeticionCaballero && ClienteMaestro.caballeroQuePide != null && ClienteMaestro.caballeroQuePide.equals(nombreCab)) {
        ...
        if (!atendido) System.out.println("[LANCE] Ningun companero tenia algo que contarle.");
    }

    private void desafiarDuelo() {
        synchronized (ClienteMaestro.lockLance) {
            if (ClienteMaestro.hayPeticionCaballero && ClienteMaestro.mensajeCaballero != null && ClienteMaestro.mensajeCaballero.equals("OFENSA_ELISABETHA")) {
    ...
    private void vigilarPortonNorte() {
        ...
        try (Socket skPorton = new Socket(ClienteMaestro.HOST, ClienteMaestro.PUERTO_PORTON);
            ...
            if (estadoLugar.equals("LUGAR_DESTRUIDO")) {
                System.out.println("[LANCE] ¡IMPOSIBLE! ¡El Portón Norte está en ruinas! No puede hacer la guardia.");
                return;
            }
    ...
    private void vigilarTaberna() {
        ...
        try (Socket skTaberna = new Socket(ClienteMaestro.HOST, ClienteMaestro.PUERTO_TABERNA);
            ...
            String estadoLugar = entrada.readUTF();
            if (estadoLugar.equals("LUGAR_DESTRUIDO")) {
                System.out.println("[LANCE] ¡DESOLADOR! ¡La Taberna es un montón de escombros humeantes! No puede vigilar.");
                return;
            }
    ...
    private void encuentroConElisabetha(DataOutputStream salida, DataInputStream entrada) throws IOException {
        ...
        if (!yaSeConocian) {
            ...
            System.out.println("[LANCE] *** LA CHISPA ADECUADA HA NACIDO *** Chispa: " + nivelChispa);
        } else {
    ...
    private void esperarElisabethaEnTaberna() {
        try (Socket skTaberna = new Socket(ClienteMaestro.HOST, ClienteMaestro.PUERTO_TABERNA);
            ...
            String estadoLugar = entrada.readUTF();
            if (estadoLugar.equals("LUGAR_DESTRUIDO")) {
                System.out.println("[LANCE] Iba a la Taberna a esperar, pero está destruida. Esperará en el portón.");
            ...
            if (resultado.equals("FINAL_FELIZ")) {
                System.out.println("[LANCE] *** FINAL FELIZ *** ¡Se reencuentra con Elisabetha!");
            }
            salida.writeUTF("DESCONECTAR");
            entrada.readUTF();
        } catch (IOException e) {
