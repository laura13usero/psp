...
/**
 * HILO ELISABETHA - CASO G: Dragon Local Synchronized.
 * Sin cambios respecto al original (el dragon la ataca via synchronized
 * desde fuera, Elisabetha no necesita metodo nuevo, el efecto se aplica
 * directamente a ClienteMaestro.chispaElisabetha desde HiloDragon).
 */
public class HiloElisabetha extends Thread {
    ...
    public void run() {
        ...
        while (!ClienteMaestro.simulacionTerminada) {
            synchronized (ClienteMaestro.lockElisabetha) {
                // Sincronizar chispa con la variable global (el dragon puede haberla cambiado)
                nivelChispa = ClienteMaestro.chispaElisabetha;
    ...
    private void visitarTaberna() {
        try {
            ...
            else { while (System.currentTimeMillis() - t0 < 5000) {
